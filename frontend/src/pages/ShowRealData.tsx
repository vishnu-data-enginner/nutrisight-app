...
const ShowRealData: React.FC = () => {
  ...
  useEffect(() => {
    const fetchRealData = async () => {
      try {
        console.log('🔍 Fetching real data for anjana.swapna...');
        
        // Get the specific user data from profiles table
          ...
          >
            ...
            <div className="mt-8 p-6 bg-green-50 dark:bg-green-900/20 rounded-xl">
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-center">
                <div>
                  <div className="text-3xl font-bold text-green-600 dark:text-green-400">{userData.total_scans}</div>
                  <div className="text-green-700 dark:text-green-300">Total Scans</div>
                </div>
                <div>
                  <div className="text-3xl font-bold text-green-600 dark:text-green-400">{userData.free_scans}</div>
                  <div className="text-green-700 dark:text-green-300">Free Scans</div>
                </div>
                <div>
                  <div className="text-3xl font-bold text-green-600 dark:text-green-400">{userData.goal_progress}%</div>
                  <div className="text-green-700 dark:text-green-300">Goal Progress</div>
                </div>
                <div>
                  <div className="text-3xl font-bold text-green-600 dark:text-green-400">{userData.username}</div>
